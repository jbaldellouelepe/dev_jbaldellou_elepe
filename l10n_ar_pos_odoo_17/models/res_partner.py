...
class ResPartner(models.Model):
    ...
    def get_all_partner_taxes(self):
        ...
        for partner in self:
            ...
            _logger.info('partner_rec {}'.format(partner_rec))
            _logger.info('arba_alicuot_ids {}'.format(partner_rec.arba_alicuot_ids))
            if partner_rec.arba_alicuot_ids:
                tags_used = [item.tag_id.id for item in partner_rec.arba_alicuot_ids]
                _logger.info('tags_used {}'.format(tags_used))
                _logger.info('invoice_repartition_line_ids {}'.format(tax_tag.invoice_repartition_line_ids))
                for line in tax_tag.invoice_repartition_line_ids:
                    _logger.info('tag_ids {}'.format(line.tag_ids))
                    for tag in line.tag_ids:
                        _logger.info('tag.id {}'.format(tag.id))
                        if tag.id in tags_used:
                            tax_repartition_lines = line.tax_id.invoice_repartition_line_ids. \
                                filtered(lambda x: x.repartition_type == 'tax')
                            _logger.info('line.tax_id.amount_type {}'.format(line.tax_id.amount_type))
